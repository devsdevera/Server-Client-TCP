...
int main(int argc, char *argv[])
{
    ...
    while(terminate == 1){
        ...
        pid_t p = fork();                       // challenge encapsulated in a couple lines.
        if(p < 0){
            error("Error: Creating a Fork");    // if not successful should perfrom steps 4 and 5 i.e just do it normally.
            loop(cfd, command, bye, terminate, get, put);
        }else if(p > 0){                        // since the client was accepted in both, the parent should close client
            close(cfd);
        }else if(p == 0){
            loop(cfd, command, bye, terminate, get, put);
        }
    ...
    while(end == 1){
        ...
        if(strcasecmp(command, bye) == 0){          // bye will not allow for any spaces just bye.
            ...
            if(is_empty(argument)){                     // bye should remain by itself, no arguments
                ...
                terminate = 0;                          // terminate the process to save memory
            }else{
